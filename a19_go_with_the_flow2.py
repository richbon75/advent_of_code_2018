'''So after analyzing the code,
it turns out that it's putting a
number in reg[3] and then adding up
every unique valid positive integer factor
in reg[0], but doing it in O(n^2) time.
This simpler version does it in O(n) time.'''
...
if __name__=="__main__":
    ...
    for instruction in cpu.prog:
        print(instruction)
    # Day two wrinkle: reg[0] starts as 1
    cpu.reg[0] = 1
    # only run enough of the code to find
    # out what our "bignumber" is
    while cpu.execute_instruction():
        # print(cpu.pc, cpu.reg)
        if cpu.pc == 1:
            print(f'The large number to be divided in reg[3] is {cpu.reg[3]}')
            bignum = cpu.reg[3]
            break
    # now sum up the factors for the answer
    factorsum = 0
    for i in range(1,bignum+1):
        if bignum%i == 0:
            factorsum += i
    print(f'Answer to part 2 is: {factorsum}')
